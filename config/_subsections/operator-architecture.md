...
![Alt text](/images/4.1.0/cryostat-architecture.dot.svg "Sample Deployment Scenario")
...
When deployed outside of **OpenShift**, the **Cryostat** <code>Service</code> is instead fronted by an instance of **OAuth2 Proxy**. This behaves very similarly to the **OpenShift OAuth Proxy** except without the integration to the cluster's internal OAuth server. Instead, users are able to configure an htpasswd file to define <code>Authorization: Basic base64(user:pass)-style</code> authentication. In this mode there is no RBAC, users either have an account and may access **Cryostat** or they have no account.
